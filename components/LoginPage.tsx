...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess, onBack, onSwitchToSignUp }) => {
  const { signInWithGoogle, signInWithOtp, loading: authLoading } = useAuth();
      ...
      <div className="min-h-screen bg-gradient-to-b from-sky-100 via-sky-50 to-[#fdfcf0] flex items-center justify-center p-6 relative overflow-hidden font-[Quicksand]">
        <div className="max-w-md w-full relative z-10 animate-fade-in">
          <div className="ghibli-card p-12 bg-white/90 shadow-xl border border-white text-center space-y-6">
            ...
            <h1 className="text-3xl font-bold text-stone-700 font-[Patrick_Hand]">Check your email</h1>
            <p className="text-stone-500">We've sent a magic link to <span className="font-bold text-stone-700">{email}</span>. Click the link to log in instantly!</p>
      ...
      <div className="max-w-md w-full relative z-10 animate-fade-in">
        ...
        <div className="ghibli-card p-8 bg-white/90 shadow-xl border border-white">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-stone-700 font-[Patrick_Hand] mb-2 tracking-wide">Welcome Back</h1>
            <p className="text-stone-500 font-medium">Enter your email to receive a magic login link</p>
          </div>

          {error && (
            <div className="mb-4 p-3 bg-rose-100 text-rose-700 rounded-lg text-sm font-bold text-center">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              ...
              className="w-full bg-sky-500 text-white py-3.5 font-bold shadow-lg shadow-sky-200 hover:bg-sky-600 hover:shadow-sky-300 transition-all h-auto"
            >
              {isLoggingIn ? 'Sending magic link...' : 'Continue'}
            </Button>
          </form>
